...
RegisterNetEvent('qb-storerobbery:server:takeMoney', function(register, isDone)
    ...
    if isDone then
        ...
        if math.random(1, 100) <= Config.stickyNoteChance then
            ...
            TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items['stickynote'], 'add')
        end
    end
end)
...
RegisterNetEvent('qb-storerobbery:server:SafeReward', function(safe)
    ...
    if luck <= 10 then
        ...
        TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items['rolex'], 'add')
        if luck == odd then
            Wait(500)
            exports['ps-inventory']:AddItem(src, 'goldbar', 1, false, false, 'qb-storerobbery:server:SafeReward')
            TriggerClientEvent('ps-inventory:client:ItemBox', src, QBCore.Shared.Items['goldbar'], 'add')
        end
    end
end)

...
RegisterNetEvent('qb-vineyard:server:getGrapes', function()
    ...
    exports['ps-inventory']:AddItem(source, 'grape', amount, false, false, 'qb-vineyard:server:getGrapes')
    TriggerClientEvent('ps-inventory:client:ItemBox', source, QBCore.Shared.Items['grape'], 'add')
end)
    ...
    if Player.PlayerData.items ~= nil then
        if grape ~= nil then
            if grape.amount >= 23 then
                ...
                TriggerClientEvent('ps-inventory:client:ItemBox', source, QBCore.Shared.Items['grapejuice'], 'remove')
                cb(true)
            else
    ...
    if Player.PlayerData.items ~= nil then
        if grape ~= nil then
            if grape.amount >= 16 then
                ...
                TriggerClientEvent('ps-inventory:client:ItemBox', source, QBCore.Shared.Items['grape'], 'remove')
                cb(true)
            else
                TriggerClientEvent('QBCore:Notify', source, Lang:t('error.invalid_items'), 'error')
                cb(false)
...
RegisterNetEvent('qb-vineyard:server:receiveWine', function()
    ...
    exports['ps-inventory']:AddItem(source, 'wine', amount, false, false, 'qb-vineyard:server:receiveWine')
    TriggerClientEvent('ps-inventory:client:ItemBox', source, QBCore.Shared.Items['wine'], 'add')
end)
...
RegisterNetEvent('qb-vineyard:server:receiveGrapeJuice', function()
    ...
    exports['ps-inventory']:AddItem(source, 'grapejuice', amount, false, false, 'qb-vineyard:server:receiveGrapeJuice')
    TriggerClientEvent('ps-inventory:client:ItemBox', source, QBCore.Shared.Items['grapejuice'], 'add')
end)

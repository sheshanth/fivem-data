...
RegisterServerEvent('AttackTransport:graczZrobilnapad', function()
	...
	TriggerClientEvent('ps-inventory:client:ItemBox', _source, QBCore.Shared.Items['markedbills'], 'add')
	...
	if chance >= 95 then
		...
		TriggerClientEvent('ps-inventory:client:ItemBox', _source, QBCore.Shared.Items['security_card_01'], 'add')
	end
	Wait(2500)
end)
